...
export default function CreateRestaurantCategoryScreen ({ navigation }) {
  ...
  const createRestaurantCategory = async (values) => {
    setBackendErrors([])
    try {
      const createdRestaurant = await CreateRestaurantCategory(values)
      showMessage({
        message: `RestaurantCategory ${createdRestaurant.name} succesfully created`,
        type: 'success',
        style: GlobalStyles.flashStyle,
        titleStyle: GlobalStyles.flashTextStyle
      })
      navigation.navigate('CreateRestaurantScreen', { dirty: true, refresh: true })
    } catch (error) {
      console.log(error)
